...
std::string &result)
{
    ...
    size_t step = a_row_counts / THREADS;
    for (size_t i = 0; i < THREADS; ++i) {
        size_t start = i * step;
        size_t end = (i == THREADS - 1) ? a_row_counts : start + step; // Ensuring the last thread covers the remainder
        threads.emplace_back(prSpSpMM<KEY, TYPE>, start, end, std::ref(A), std::ref(B), std::ref(res));
    }
